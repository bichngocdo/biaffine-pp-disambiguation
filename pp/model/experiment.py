...
import pp.model.data_reader as data_reader
import pp.model.data_writer as data_writer
from pp.model.data_converter import DataConverter
from pp.model.data_encoder import DataEncoder
from pp.model.evaluator import Stats
from pp.model.network import PPDisambiguator
...
class Experiment(object):
    ...
    def init_data_model(self, raw_data):
        ...
        self.args.no_topological_fields = len(self.data_encoder.topological_field_encoder)

        print('No. words:                %d' % self.args.no_words)
        print('No. tags:                 %d' % self.args.no_tags)
        print('No. characters:           %d' % self.args.no_chars)
        print('No. topological fields:   %d' % self.args.no_topological_fields)

        self.save_data_model()

    def init_network(self, session):
        self.network = PPDisambiguator(self.args)
        self.network.initialize_global_variables(session)
    ...
    def __train_old(self, train_dataset, dev_dataset, interval,
        ...
        for vars in batches:
            ...
            self.train_stats.update(loss, end - start, vars[-1], outputs['prediction'])
        ...
        for batch in batches:
            ...
            for mini_batch in mini_batches:
                ...
                try:
                    ...
                    print(mini_batch)
                    print([v.shape if v is not None else None for v in vars])
                    print(exp)
                    exit(-1)
            ...
            self.train_stats.update(loss, end - start, batch[-1], outputs['prediction'])
    ...
    def __validate(self, dev_dataset, validate_batch_size, session):
        ...
        for batch in batches:
            ...
            self.dev_stats.update(loss, end - start, vars[-1], outputs['prediction'])
        ...
        summary = self.network.make_train_summary(session, stats['train_acc'], stats['train_sent_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)
        summary = self.network.make_dev_summary(session, stats['dev_loss'], stats['dev_acc'], stats['dev_sent_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)

        self.save()
        self.save_network(session)

        if stats['dev_acc'] > self.best_score:
            self.best_score = stats['dev_acc']
    ...
    def __evaluate(self, dev_dataset, eval_batch_size, session):
        ...
        for batch in batches:
            ...
            self.dev_stats.update(loss, end - start, vars[-1], outputs['prediction'])
            results.extend(outputs['prediction'])
...
def evaluate(args):
    ...
    dev_raw_data = data_reader.read_json(args.test_file)
    ...
    if args.output_file is not None:
        data_writer.write_json(args.output_file, dev_raw_data, results)
...
if __name__ == '__main__':
    ...
    group.add_argument('--use_topological_fields', type=str2bool, default=False,
                       help='use topological fields')
    group.add_argument('--use_scores', type=str2bool, default=False,
                       help='use scores')
    ...
    group.add_argument('--validate_batch_size', type=int, default=10000,
                       help='mini-batch size in number of tokens')
    ...
    group.add_argument('--topological_field_dim', type=int, default=20,
                       help='topological field dimension')
    ...
    group.add_argument('--mlp_dim', type=int, default=500,
                       help='MLP dimension')
    group.add_argument('--clf', choices=['bilinear', 'mlp'],
                       default='bilinear', help='scoring classifier')
    group.add_argument('--clf_hidden_dim', type=int, default=500,
                       help='scoring hidden dimension')
    ...
    group.add_argument('--loss', choices=['cross_entropy', 'ranking_all', 'ranking_max'],
    ...
    parser_eval.add_argument('--batch_size', type=int, default=100,
    ...
    parser_infer.add_argument('--batch_size', type=int, default=32,
                              help='mini-batch size in number of sentences')
